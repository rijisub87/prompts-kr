  ...
  return (
    <div ref={wrapRef} className="relative w-full max-w-md">
      <input
        ...
        className="w-full rounded border bg-white px-3 py-1.5 text-sm placeholder:text-slate-400 focus:border-emerald-400 focus:outline-none"
      />
      {open && q.trim() && (
        <div className="absolute left-0 right-0 top-full z-30 mt-1 max-h-96 overflow-y-auto rounded border bg-white shadow-lg">
          {results.length === 0 ? (
              ...
              {results.map(item => (
                <li key={item.slug}>
                  <Link
                    ...
                    className="block border-b px-3 py-2 last:border-b-0 hover:bg-slate-50"
                  >
