  ...
  return (
    <article className="space-y-8">
      <header>
        ...
        <div className="mt-2 text-xs text-emerald-700">{CATEGORY_KO[guide.category]}</div>
        <h1 className="mt-1 text-3xl font-bold">{guide.title}</h1>
        <p className="mt-2 text-base text-slate-600">{guide.summary}</p>
      </header>

      <section
        className="prose prose-sm max-w-none prose-headings:font-bold prose-h2:mt-8 prose-h2:text-xl prose-h3:mt-4 prose-h3:text-lg prose-pre:bg-slate-900 prose-pre:text-slate-100"
        dangerouslySetInnerHTML={{ __html: guide.bodyHtml }}
      />

      {relatedPrompts.length > 0 && (
        <section className="rounded border-2 border-emerald-200 bg-emerald-50 p-5">
          ...
          <ul className="space-y-2">
            {relatedPrompts.map(p => (
              <li key={p.slug}>
                <Link
                  href={`/p/${p.slug}`}
                  className="block rounded bg-white px-3 py-2 text-sm hover:bg-slate-50"
                >
