...
const GROUP_TONE: Record<GlossaryGroup, string> = {
  '기초':         'border-slate-300 bg-slate-50',
  '모델':         'border-blue-200 bg-blue-50',
  '사용 기법':    'border-emerald-200 bg-emerald-50',
  '평가·안전':    'border-amber-200 bg-amber-50',
  '최신 트렌드':  'border-purple-200 bg-purple-50',
};
              ...
              {terms.map(t => (
                ...
                >
                  <div className="flex flex-wrap items-baseline gap-2">
                    <h3 className="text-base font-bold text-slate-900">{t.ko}</h3>
                    <span className="text-xs text-slate-500">{t.en}</span>
                  </div>
                  <p className="mt-2 text-sm font-medium text-slate-800">{t.short}</p>
                  <p className="mt-2 text-sm text-slate-700">{t.body}</p>
                  {t.example && (
                    <p className="mt-2 rounded bg-white/60 p-2 text-xs italic text-slate-700">
                      예: {t.example}
                    </p>
                  )}
                </div>
