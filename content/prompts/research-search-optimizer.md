---
title: 검색어·쿼리 최적화 (연산자 활용)
slug: research-search-optimizer
category: research
platform: [공통]
language: 한국어
addedAt: 2026-05-31
source:
  name: Google Search Help — Refine web searches
  url: https://support.google.com/websearch/answer/2466433
  author: Google
variables:
  - { name: GOAL, label: "리서치 목표" }
  - { name: LANGUAGE, label: "언어" }
  - { name: DATE_RANGE, label: "기간 제한 (없으면 '없음')" }
---

```
당신은 검색 전문가다. 내 리서치 목표를 받아 효과적인 검색 쿼리 세트를 만들어라.

리서치 목표: [GOAL]
검색엔진: Google / Google Scholar / Perplexity / 네이버
언어: [LANGUAGE]
기간 제한: [DATE_RANGE]

다음을 출력하라:

1. 핵심 키워드 3종(좁은/중간/넓은)
2. 부울/연산자 쿼리 5개. 각각 다음 연산자 중 적절히 사용:
   - "정확한 구문"
   - site:도메인
   - filetype:pdf
   - intitle:키워드
   - -제외어
   - OR
   - before:YYYY-MM-DD / after:YYYY-MM-DD
3. 각 쿼리가 어떤 종류의 결과를 가져올지 한 줄 설명
4. 1차 결과에서 발견될 가능성이 높은 "다음 단계 쿼리" 3개
5. 한국어/영어 검색 결과 차이가 예상되면 양쪽 쿼리 모두 제시
```

검색 자체를 못 하는 AI라도 이 프롬프트로 쓸 만한 검색 쿼리 세트를 뽑아 두면, 그걸 그대로 구글·구글 스칼라·네이버에 붙여 넣어 직접 리서치하는 시간을 줄일 수 있습니다. 좁은/중간/넓은 키워드를 동시에 주고, 정확 구문·site:·filetype:·intitle:·제외어·OR·기간 연산자를 섞은 쿼리를 5개 만들어 주므로, 막연히 키워드만 치다 헤매는 단계를 건너뛰게 됩니다. 활용 팁: GOAL을 "X가 Y에 미치는 영향에 대한 2023년 이후 학술 근거"처럼 구체적으로 쓸수록 쿼리가 날카로워지고, 정부·학회 자료가 필요하면 site:go.kr이나 site:edu를 직접 추가 요청하세요. 연산자 동작은 검색엔진마다 조금씩 다르니(특히 네이버), 핵심 쿼리는 실제로 돌려 보고 결과 수를 확인하는 게 좋습니다.
